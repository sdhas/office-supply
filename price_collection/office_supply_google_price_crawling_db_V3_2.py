...
CLIENT = 'officesupply'
OPEN_INPUT_BATCH_SIZE = 100
OUTPUT_BATCH_SIZE = 10
RETRY_INPUT_BATCH_SIZE = 10
...
def update_input_to_retry(input: Request, status: str):
    ...
    post_header = {'Content-Type': 'application/json', "sap-device-name": device_name}
...
def update_products(products: list):
    ...
    product_json = json.dumps(products, cls=ModelEncoder)

    log_and_console_info(f"Inserting products : {product_json}")
    ...
    if(post_response.status_code == 200):
        log_and_console_info("Output save SUCCESS!")
    else:
        log_and_console_error("Output save FAILED!")
...
def main():
    ...
    try:        
        ...
        if(len(sys.argv) < 2):
            ...
            logging.basicConfig(filename=f'{sys.argv[1]}/app_log.txt', format='%(asctime)s %(message)s', level=logging.INFO)

            log_and_console_info(f'Received {len(sys.argv)} arguments {sys.argv}')
